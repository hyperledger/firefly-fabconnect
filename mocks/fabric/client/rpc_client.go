...
func (_m *RPCClient) QueryTransaction(channelId string, signer string, txId string) (*peer.FilteredTransaction, error) {
	ret := _m.Called(channelId, signer, txId)

	var r0 *peer.FilteredTransaction
	if rf, ok := ret.Get(0).(func(string, string, string) *peer.FilteredTransaction); ok {
		r0 = rf(channelId, signer, txId)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*peer.FilteredTransaction)
		}
	}
